...
def start_instance():
    try:
        ...
        for _ in range(10):  # Check every 3 seconds for up to 30 seconds
            time.sleep(3)
            response = ec2.describe_instance_status(InstanceIds=[INSTANCE_ID])
            statuses = response.get("InstanceStatuses", [])
            if statuses and statuses[0]["InstanceState"]["Name"] == "running":
        ...
        print("Instance start initiated, but not confirmed as running yet.")
        return {
            "status": "pending",
            "message": "Instance start initiated, but status is still pending.",
        }
